...
export class DashboardService {
  ...
  private async examPassRate(schoolId: string, examId: string, branchId?: string) {
    ...
    for (const { obtained, max } of totals.values()) {
      if (max > 0 && (obtained / max) * 100 >= PASS_THRESHOLD_PERCENT) passing++;
    }
    return { studentsGraded: totals.size, passing, rate: (passing / totals.size) * 100 };
  }

  async getSummary(schoolId: string, branchId?: string) {
    ...
    // --- Success rate: pass rate of the most recent exam vs the one before it ---
    ...
    // --- Attendance breakdown for the most recent marked date ---
    ...
    const countStatus = (status: string) => latestDayAttendance.filter((a) => a.status === status).length;
    ...
    const statusCount = (status: string) =>
      submissionCounts.find((s) => s.status === status)?._count ?? 0;
    ...
    // --- Two secondary progress bars: 30-day attendance rate, this-month fee collection rate ---
    const attendance30Total = last30DaysAttendance.length || 1;
    const attendance30Present = last30DaysAttendance.filter(
    ...
    // --- Top students by average mark percentage ---
    ...
    // --- Fee collection by month (last 6 periods) ---
    ...
    // --- Recent assignments with submission/grading progress ---
